...
import com.mycompany.sgfc2.view.ver.verCaminhao;
import com.mycompany.sgfc2.view.ver.verFuncionario;
...
public class CargaConsulta extends javax.swing.JInternalFrame {
    ...
    private void btnExibirConsultaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExibirConsultaActionPerformed
        try {
            ...
            if (cg.getConferidoPor() != null) {
                param.put("cargaConfPor", cg.getConferidoPor().getNome());
            } else {
                param.put("cargaConfPor", "");
            }

            if (cg.getAutorizadoPor() != null) {
                param.put("cargAutPor", cg.getAutorizadoPor().getNome());
            } else {
                param.put("cargAutPor", "");
            }

            if (c != null) {
                param.put("caminhao", "Marca: " + c.getMarca() + " - Ano: " + c.getAnoModelo()
                        + " - Capacidade: " + c.getCapacidadeCarga() + " - Ultima revisão: " + c.getUltimaRevisao().format(formatter));
                param.put("nomeMot", c.getMotorista().getNome());
                param.put("cnhMot", c.getMotorista().getCnh());
                param.put("cnhMot", c.getMotorista().getCnh());
            } else {
                param.put("caminhao", "");
                param.put("nomeMot", "");
            ...
            criaRelatorio("CargaCompletoJasper.jasper", param);
        } catch (Exception e) {
        }
